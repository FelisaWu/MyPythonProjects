...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH, paddle_height=PADDLE_HEIGHT,
        ...
        for i in range(brick_cols):
            for j in range(brick_rows):
                self.bricks = GRect(brick_width, brick_height, x=0+i*brick_spacing+i*brick_width,
                                    y=brick_offset+j*brick_height+j*brick_spacing)
                self.bricks.filled = True
                self.bricks.fill_color = 'grey'
                self.window.add(self.bricks)
        # Initialize our mouse listeners
        onmousemoved(self.move_paddle)
        onmouseclicked(self.change_ball_position)
        self.game_start = False
        self.label = GLabel('Game Over!', window_width/2, window_height/2)
    ...
    def change_ball_position(self, evenr):
